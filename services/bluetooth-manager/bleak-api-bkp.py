...
class BluetoothManager:
    ...
    async def connect(
        ...
        client = None
        try:
            # Use cached device info if available, otherwise create from address
            if address in self._discovered_devices:
                device_info = self._discovered_devices[address]
                device = BLEDevice(
                    address=address,
                    name=device_info.name or "",
                    details={"uuids": device_info.service_uuids},
            ...
            # Check if this is a D-Bus/backend error (BlueZ not available or socket not found)
            # These are actual backend configuration issues
            if ("no such file or directory" in error_msg and "dbus" in error_msg) or \
               ("connection refused" in error_msg and "dbus" in error_msg) or \
               ("failed to connect to session bus" in error_msg) or \
               ("cannot find bluetooth adapter" in error_msg and "hci" in error_msg):
                raise HTTPException(
                    status_code=503,
                    detail="Bluetooth backend unavailable. Ensure BlueZ D-Bus is configured and the container has DBUS_SYSTEM_BUS_ADDRESS set."
                )
            # "'path'" in error typically means D-Bus couldn't find the device object
            # This is a device-specific issue, not a backend issue
            if "'path'" in error_msg:
                raise HTTPException(status_code=404, detail=f"Device not found: {e}")
            # Check if pairing is needed
            if "authentication" in error_msg or "encrypted" in error_msg or "pairing" in error_msg:
                if client is None:
                    client = BleakClient(address, timeout=timeout)
                try:
                    await client.pair()
                    ...
                    raise HTTPException(status_code=500, detail=f"Connection failed after pairing: {str(pair_error)}")
